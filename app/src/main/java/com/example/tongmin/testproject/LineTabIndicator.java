...
public class LineTabIndicator extends HorizontalScrollView {
	...
	private int indicatorColor = 0xFFFF7975;
	private int underlineColor = 0xFFE5E5E5;
	private int dividerColor = 0xFFFF7975;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (indicatorOnTop) {
			...
			RectF rect = new RectF(lineLeft, height - indicatorHeight, lineRight, height);
			canvas.drawRoundRect(rect, 5.0f, 5.0f, linePaint);
		...
		if (enableDivider) {
			...
			for (int i = 0; i < tabCount - 1; i++) {
				...
				RectF rect = new RectF(tab.getRight() - dividerPadding, dividerPadding, tab.getRight(),
						dividerPadding + 4);
				canvas.drawRoundRect(rect, 10.0f, 10.0f, diviPaint);
			}
		}
	}
...
//			int paddingLeftOrRight = (int) (padding / getResources().getConfiguration().fontScale);
			mTabText.setGravity(Gravity.CENTER);
			mTabText.setPadding(padding, 0, padding, 0);
			LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
//			params.setMargins(paddingLeftOrRight, 0, paddingLeftOrRight, 0);
	...
	private void addTab(final int position, String title) {
		...
		mTabsContainer.addView(tab, position,
				enableExpand ? new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1.0f)
	...
	private void tabSelect(int index) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (isSelected) {
				// ((TabView)
				// child).getTextView().setTextColor(textSelectedColor);
				TextView tv = ((TabView) child).getTextView();
				TextPaint tp = tv.getPaint();
				tp.setFakeBoldText(true);
			} else {
				// ((TabView)
				// child).getTextView().setTextColor(textUnselectColor);
				TextView tv = ((TabView) child).getTextView();
				TextPaint tp = tv.getPaint();
